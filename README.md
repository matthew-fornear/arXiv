...
**Note:** The main script (`arxivCollector.py`) uses the official arXiv API and requires no additional dependencies beyond the standard library. The web scraping version (`arxivCollector_web.py`) requires `requests` and `beautifulsoup4`.

## Scripts

There are two scripts available:

1. **`arxivCollector.py`** (Recommended) - Uses the official arXiv API
   - More reliable and faster
   - No cookies or headers needed
   - Uses Atom XML format
   - Respects API rate limits (3 second delays)

2. **`arxivCollector_web.py`** - Uses web scraping (legacy)
   - Requires cookies and headers in `.env` file
   - More fragile, subject to HTML changes
   - Useful if API doesn't work for your use case

## Environment (Web Scraping Only)

For the web scraping version (`arxivCollector_web.py`), you need a `.env` file:

1. Copy `.env.example` to `.env`.
2. Replace the placeholder values with your actual cookies and headers.
   - `ARXIV_COOKIE` must contain the full cookie header string.
   - Other headers are optional but may improve compatibility.

Keep `.env` out of version control—`.gitignore` already takes care of this.

**The API version (`arxivCollector.py`) does NOT require a `.env` file.**

## Running

### Query-Based Collection (API Version)
...
- `--query` - Query string (required for query mode)
- `--max-results` - Results per API call (default: 2000, max: 2000)
- `--sort-by` - Sort field: `relevance`, `lastUpdatedDate`, or `submittedDate` (default: `submittedDate`)
...
**Note:** The API version uses date range queries (`submittedDate:[YYYYMMDDHHMM+TO+YYYYMMDDHHMM]`) and supports up to 30,000 results per query. For larger collections, use `--collect-all` which iterates by month.
